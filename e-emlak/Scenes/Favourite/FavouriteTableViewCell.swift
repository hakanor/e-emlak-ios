...
class FavouriteTableViewCellModel {
    let title: String
    let imageURL: URL?
    var imageData: Data?
    var url: String?
...
class FavouriteTableViewCell: UITableViewCell {
    ...
    var delegate: historyButtonClickDelegate?
    var cellIndex: IndexPath?
    
    var model = FavouriteTableViewCellModel(title: "", imageURL: URL(string: ""), url: "", urlToImage: "", publishedAt: "", description: "", content: "", sourceName: "")
    ...
    private lazy var priceHistoryButton: UIButton = {
        let button = UIButton(type: .custom)
        let image = UIImage(systemName: "pencil")
        button.setTitleColor(themeColors.grey, for: .normal)
        button.setTitleColor(themeColors.lightGrey, for: .selected)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 12)
        button.backgroundColor = .clear
        let title = "Fiyat Geçmişi"
        let attributes: [NSAttributedString.Key: Any] = [.font: UIFont.systemFont(ofSize: 14, weight: .regular), .underlineStyle: NSUnderlineStyle.single.rawValue]
        button.setAttributedTitle(NSAttributedString(string: title, attributes: attributes), for: .normal)
        button.addTarget(self, action: #selector(handlePriceHistoryButton), for: .touchUpInside)
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        [adImage, titleLabel, bookmarkIcon, locationLabel, priceLabel, locationIcon, priceHistoryButton] .forEach(containerView.addSubview(_:))
        ...
        priceHistoryButton.anchor(left: adImage.rightAnchor, bottom: containerView.bottomAnchor, paddingLeft: 20, paddingBottom: 10)
    ...
    // MARK: - Selectors
    @objc private func handlePriceHistoryButton() {
        
        self.delegate?.priceHistoryButtonClicked(cellIndex!.row, adId: self.ad.adId)
    ...
    func configureCell(title:String, price:String, location:String, url:String, ad:Ad){
        ...
        coreDataService.updateAdInCoreData(ad: ad)
        checkBookmark()
        
        if bookmarkBool {
            coreDataService.checkPriceChange(adId: ad.adId)
        }
