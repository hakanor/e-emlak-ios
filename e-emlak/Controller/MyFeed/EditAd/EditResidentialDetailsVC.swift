//
//  EditResidentialDetailsVC.swift
//  e-emlak
//
...
class EditResidentialDetailsVC: UIViewController{
    ...
    @objc func handleNextButton(){
        ...
        if checkTextFields() != "" {
            ...
            let dict = [
                ...
                "price":price,
                "squareMeter":meter,
                "squareMeterNet":meterNet,
                "numberOfRooms": Int(numberOfRooms) ?? 0,
                "numberOfFloors" : Int(numberOfFloors) ?? 0,
                "numberOfBathrooms" : Int(numberOfBathrooms) ?? 0,
                "floorNumber" : Int(floorNumber) ?? 0,
    ...
    private func setData(){
        ...
        self.squareMeterNetTextField.text = String(self.ad.squareMeterNet)
        self.squareMeterTextField.text = String(self.ad.squareMeter)
        self.ageOfBuildingTextField.text = String(self.ad.ageOfBuilding)
        self.floorNumberTextField.text = String(self.ad.floorNumber)
        self.numberOfFloorsTextField.text = String(self.ad.numberOfFloors)
        self.numberOfRoomsTextField.text = String(self.ad.numberOfRooms)
        self.numberOfBathroomsTextField.text = String(self.ad.numberOfBathrooms)
        self.heatingTextField.text = self.ad.heating
        
        let urls = self.ad.images
    ...
    private func checkTextFields() -> String {
        ...
        guard squareMeterNetTextField.text != "" else { return "Metrekare(Net) alanı boş olamaz."}
        guard ageOfBuildingTextField.text != "" else { return "Bina yaşı alanı boş olamaz."}
        guard floorNumberTextField.text != "" else { return "Daire kat sayısı alanı boş olamaz."}
        guard numberOfRoomsTextField.text != "" else { return "Oda sayısı alanı boş olamaz."}
        guard numberOfBathroomsTextField.text != "" else { return "Banyo sayısı alanı boş olamaz."}
...
extension EditResidentialDetailsVC: PHPickerViewControllerDelegate {
...
extension EditResidentialDetailsVC: UICollectionViewDataSource, UICollectionViewDelegate {
